...
public class ElectionApiServiceTest
{
    ...
    public async Task GetElection_ValidId_ReturnsElection()
    {
        ...
        _handler.Protected().Setup<Task<HttpResponseMessage>>("SendAsync",
            ItExpr.Is<HttpRequestMessage>(request => request.Method == HttpMethod.Get && request.RequestUri != null && request.RequestUri.AbsoluteUri ==  $"{_baseUrl}/api/Election/{election.Id}"),
