...
public class PbEngineController(
    IElectionService _electionService,
...
) : Controller
{
        ...
        foreach (var pythonVoter in voters)
        {
            ...
            for (int i = 0; i < pythonVoter.selectedProjects.Count; i++)
            {
                ...
                var score = new Scores()
                    { Grade = degree, Voter_Id = createdVoter.Id, Project_Id = projectToIdMap[project] };
    ...
    public async Task<Dictionary<string, float>> GetAverageSatisfaction(Guid resultId, [FromBody] List<int> sats)
    {
        ...
        {
            selectedProjects = election.SubmittedProjects
                .Where(p => v.Votes.Select(r => r.Project_Id).ToList().ToList().Contains(p.Id)).ToList().ToList()
